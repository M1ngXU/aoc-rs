...
fn two() {
	let p = sb(
		LLE,
		tuple((
			delimited(t!("Monkey "), map_parser(take_until(":"), pn), t!(":" @)),
			preceded(tag("  Starting items: "), pu(LE, sb(", ", pn))),
			delimited(
				tag("  Operation: new = old "),
				pair(
					alt((tag("*"), tag("+"))),
					preceded(
						tag(" "),
						alt((map(t!("old"), |_| -1), map_parser(take_until(LE), pn))),
					),
				),
				t!(),
			),
			preceded(tag("  Test: divisible by "), pu(LE, pn)),
			preceded(tag("    If true: throw to monkey "), pu(LE, pn)),
			preceded(tag("    If false: throw to monkey "), pn),
		)),
	);
	let mut s: Vec<(isize, Vec<isize>, (&str, isize), isize, isize, isize)> = pi!(p);
	let mut mi = vec![0; s.len()];
